...
/**
Note: Use finished code in Exercise 2

Exercise 3. Rather than returning the first error found, return back a single error that con‐
tains all errors discovered during validation. Update the code in main to properly
...
func main() {
	...
	for d.More() {
		...
		if err != nil {
			// use a type switch to detect an error that can be unwrapped into multiple err
			switch err := err.(type) {
			case interface{ Unwrap() []error }:
				// if we have one use to walkthrough errs and build a single string
